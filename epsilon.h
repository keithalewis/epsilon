...
template <class X>
using IsArithmetic =
    typename std::enable_if<std::is_arithmetic<X>::value>::type;

namespace fms {

template <size_t N, class X = double, class = IsArithmetic<X>> class epsilon {
  ...
  public:
    ...
    // x I
    epsilon(const X &x) : a(X(0), N)
    {
        a[0] = x;
    }
    // x0 I + x1 epsilon
    epsilon(const X &x0, [[maybe_unused]] const X &x1) : a(X(0), N)
    {
        a[0] = x0;
        if constexpr (N > 1)
            a[1] = x1;
    }
    epsilon(const X *px) : a(px, N)
    {
    }
    epsilon(std::initializer_list<X> il) : a{il}
    {
    }
...
inline fms::epsilon<N, X> operator+(fms::epsilon<N, X> a,
                                    const fms::epsilon<N, X> &b)
{
    return a += b;
}
