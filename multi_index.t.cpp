...
template <size_t N> multinomial epsilon(size_t i)
{
    multi_index ei(N);
    ei[i] = 1;
...
template <size_t N> multinomial constant(double x)
{
    multi_index c(N); // all 0's

    return multinomial({{c, x}});
}
template <size_t N> multinomial value(double x)
{
...
static int test_epsilon()
{
    ...
    auto ce2 = ce * ce; // (1 + e0 + e1 + e0 e1) +
                        ...
                        // = 1 + 2e0 + 2e1 + e0^2 + 4 e0 e1 + e1^2 + 2 e0^2 e1 +
                        // 2 e0 e1^2 + e0^2 e1^2
    assert(ce2[i({0, 0})] == 1);
    assert(ce2[i({1, 0})] == 2);
    assert(ce2[i({0, 1})] == 2);
